...
Get-VMHost $VMHost | Get-VMHostNetwork | Set-VMHostNetwork -DnsAddress $nameserver1,$nameserver2 -Domain $domainname -SearchDomain $domainname

#Get-VMHost $VMHost | Get-VMHostNetwork | Set-VMHostNetwork -DnsAddress $nameserver1 -Domain $domainname -SearchDomain $domainname
#如果单 DNS 服务器，使用此行
}
