...
#修改 argocd-server service 的类型，从默认的 ClusterIP 改为 NodePort，方便从外部访问
kubectl patch service argocd-server -p '{"spec": {"type": "NodePort"}}'
```

查看 NodePort 访问端口：

```
[root@k8s-m01 ~]# kubectl get svc argocd-server
NAME            TYPE       CLUSTER-IP     EXTERNAL-IP   PORT(S)                      AGE
argocd-server   NodePort   10.108.58.41   <none>        80:31480/TCP,443:31974/TCP   6m23s
```

![image-20231027133814693](../../../pics/image-20231027133814693.png)
...
```shell
# 如无法下载可在链接前加上 https://ghproxy.com
