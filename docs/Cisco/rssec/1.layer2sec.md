...
Unknow Unicast Flooding：不知道目的的单播包(目标mac没有和任何接口对应的mac相关联)，SW会向所有端口泛洪

 攻击者伪造数据包，改源MAC地址，每次交换机都会记住此源地址和端口对应关系，造成溢出，之前存储的**正确的CAM**对应关系会丢失。
...
未知单播帧（*此单播帧**mac**地址唯一，但在交换机上无记录*）攻击:
...
在接口下启用switchport block unicast

！只会阻止进来的数据



在发送单播帧的端口启用switchport block multicast也能阻止泛洪

3. arp包泛洪攻击（ARP包可能合法也可能不合法）

直接启用arp inspection，可以限制arp包数量。
