...
然后编辑Server，在Network Security 中基于IP组添加Deny 策略。
...
编辑Server，在Advanced 中勾选“Use VIP as SNAT”，此选项仅支持Active/Standby高可用模式，不支持N+M或者Active/Active。
...
编辑Server，在Advanced 中设置“SNAT IP address”，注意设置的 SNAT 地址池必须和 VIP 在同一个网段。
