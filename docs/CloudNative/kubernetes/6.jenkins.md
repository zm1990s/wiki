...
mysql -h 10.39.217.206 -u root -P 3306 -pAdmin@2023 -e "show databases;"
```
...
在本地创建下列文件，修改文件中的 IP 地址：

```shell
mkdir /tmp/ruoyi
cat > /tmp/ruoyi/ruoyi-gateway-dev.yml <<EOF
spring:
  redis:
    host: 10.10.50.66
    port: 30078
...
cat >/tmp/ruoyi/ruoyi-auth-dev.yml << EOF
spring:
  redis:
    host: 10.10.50.66
    port: 30078
...
cat >/tmp/ruoyi/ruoyi-system-dev.yml << EOF
    ...
    dynamic:
      ...
      datasource:
          master:
            ...
            url: jdbc:mysql://ruoyi-mysql.ruoyi:3306/ry-cloud?useUnicode=true&characterEncoding=utf8&zeroDateTimeBehavior=convertToNull&useSSL=true&serverTimezone=GMT%2B8
            username: root
            password: Admin@2023
...
将上述三个文件所在目录 ruoyi 压缩为 zip 文件

```shell
zip ruoyi.zip ruoyi/*
```

然后在 nacos 的配置列表中进行导入。

![image-20231206142600567](../../../pics/image-20231206142600567.png)
...
- 创建 git 秘钥，此秘钥用于拉取代码（此处使用 Linux 生成了一个 ssh 密钥对，公钥添加到了 gitlab，私钥放在 jenkins 中），记录 ID：
...
# 将主机的 ca 证书挂载给容器，保证 dind 可以访问私有 harbor
                - name: hostcerts
                  hostPath:
...
''') {
    node(POD_LABEL) {
        stage('拉取代码') {
            git credentialsId: 'b4aa2f30-e922-423d-9ede-eccf6dc75a6d', url: 'https://gitlab.halfcoffee.com/root/ruoyi.git'
            container('maven') {
                stage('代码编译') {
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: ruoyi-gateway
        image: harbor.halfcoffee.com/ruoyi/ruoyi-gateway:6ff45b2-25
        ports:
        - name: http
          containerPort: 8080
          protocol: TCP
