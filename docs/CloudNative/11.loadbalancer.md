...
AKO 的 Helm charts 可以在 myvmware 中获取，或者通过 VMware 公开的 Helm 仓库获取：
...
通过下列命令来部署 AKO（Helm 二进制可以在[此位置](https://github.com/helm/helm/releases)进行下载，参数与实际环境一致即可）：
...
应用上述 YAML 后，Avi 控制器会自动检查此 CRD 的配置是否合规，如果合规，则其状态会变为 Accepted，这种方式可以很好避免误配置，方便排错。
...
接着我们多次访问该虚拟服务，可以看到日志中会有详细的请求记录：
