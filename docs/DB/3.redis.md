...
![image-20230504144409207](../../pics/image-20230504144409207.png)

### Sentinel 和 Client 在一起的部署模式

Quorum=2

![image-20230504150141663](../../pics/image-20230504150141663.png)
...
![image-20230504144337821](../../pics/image-20230504144337821.png)
...
每个 Redis node 会有一组 hash slot，hash slot 可以动态分配（通过 resharding 或者 rebalance 操作）。在集群模式下，每个 Redis 只允许单个逻辑数据库工作，即编号为 0 的数据库。
...
| ------------------------- | -------------- | ------------- | ------------------------------------------------------ |
| Multiple Logical Databash | Yes            | No            | 普通模式的 Redis 提供 16 个逻辑数据库                  |
| 一致性保证                | No             | No            | 可能存在写入请求丢失                                   |
| 水平扩展                  | No             | Yes           | 最大 1000 节点，线性扩展                               |
| 复制                      | Yes            | Yes           | 均支持异步复制                                         |
...
https://www.baeldung.com/redis-sentinel-vs-clustering#:~:text=Redis%20Sentinel%20combines%20forces%20with,failover%2C%20configuration%20management%2C%20etc
