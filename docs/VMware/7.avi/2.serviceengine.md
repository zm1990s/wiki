...
| N+M            | 适合于单个 SE 能够满足单个 VS 的性能，以及应用对高可用要求不高，可以接受短暂的服务中断的场景 | 其中 N 代表运行所有 VS 需要的最小 SE 数量，M  代表允许的 SE 故障数。两者共同决定了最终会有多少个 SE 创建出来 | 比较灵活，资源利用率较高，允许多个节点故障 |
