...
在集群模式下，所有 Single key command 均可以支持，复杂的 multi-key 操作需要在 key 属于同一个 hash slot 中时支持。

在集群模式下，每个 Redis 只允许单个逻辑数据库工作，即编号为 0 的数据库。不支持 SELECT 命令。
...
默认 Redis 使用 6379 端口提供服务，Redis Cluster 则使用 16379 端口作为 cluster bus port。可通过 `cluster-port`来覆写此配置。

Redis 通过 cluster bus 来进行故障检测，配置变更，故障切换，自动发现其他节点等。

集群模式下节点间使用 gossip protocol 来传播集群信息，自动发现其他节点；发送 PING 来检查其他节点是否正常。

Redis 节点在集群模式下不支持代理请求，而是使用 MOVED 或者 ASK 重定向错误来通知 Client 应该访问哪个节点来获取数据。理论上 Client 可以将请求发给任意节点，在需要的时候重定向请求，但是为了性能，Client 可以记录 Key 与节点的映射关系。
...
在 Redis Cluster 模式下，Redis Client 会自动的将请求发送到负责某 hash slot 的节点，所以这种模式下不需要负载均衡。

例如 Client（10.10.50.235）发送 ` get key1` 的请求给 10.10.50.234，而此 key 的 hash slot 归 239 负责，则会返回 `MOVED 9189 10.10.50.239:6379` 信息：

![image-20230508164737055](../../pics/image-20230508164737055.png)

 之后 Client 会自动和 239 建立连接来获取相应的 value：

![image-20230508164942665](../../pics/image-20230508164942665.png)



但是在 Redis Cluster 下，也可以为**每组 Master/Replica** 配置一个负载均衡，这样可以减少 Client 端 Redis Pool 的配置，但这样的优化很有限。

具体方式和哨兵模式下 Write 的负载均衡类似，可以用  `info replication` 来检查节点是否为 master，然后将其设置为 active 的成员。具体配置笔者不再赘述。
