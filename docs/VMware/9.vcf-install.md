...
l 在ESXi上创建两个端口组，每个端口组的Active Uplink仅使用一个vmnic，另一个vmnic作为Standby

l 为Edge设置Uplink配置文件，定义Named Teaming Policy

l 为Edge创建一个 VLAN 传输区域，设置传输区域级别的Named Teaming Policy绑定

l 在部署Edge时创建一个NVDS，分别关联到第一步的两个端口组，添加两个接口，每个接口对应到上一步的两个Uplink

l 为Edge创建两个 Uplink Segment，关联到Edge的VLAN传输区域，通过绑定策略限制每个 Segment 只走一个Uplink
